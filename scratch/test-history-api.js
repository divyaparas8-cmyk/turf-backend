...
async function testHistoryAPI() {
  try {
    ...
    console.log('--- FETCHING OWNER DASHBOARD HISTORY DATA ---');
    ...
    console.log('GET /dashboard/history status:', res.status);
    const logs = res.data?.data?.allLogs || [];
    console.log(`Found ${logs.length} logs in dashboard history:`);
    logs.slice(0, 5).forEach(b => {
